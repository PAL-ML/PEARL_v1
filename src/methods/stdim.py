...
class CLIPInfoNCESpatioTemporalTrainer(Trainer):
    def __init__(self, encoder, config, device=torch.device('cpu'), wandb=None):
        super().__init__(encoder, wandb, device)
        self.config = config
        self.patience = self.config["patience"]
        self.classifier1 = nn.Linear(self.encoder.hidden_size, self.encoder.patch_emb_size).to(device)  # x1 = global, x2=patch, n_channels = 32
        self.classifier2 = nn.Linear(self.encoder.patch_emb_size, self.encoder.patch_emb_size).to(device)
        self.epochs = config['epochs']
        self.batch_size = config['batch_size']
    ...
    def do_one_epoch(self, epoch, episodes):
        ...
        for x_t, x_tprev in data_generator:
            ...
            f_t = f_t_maps['patch']
            loss2 = 0.
            predictions = self.classifier2(f_t)
            positive = f_t_prev
            logits = torch.matmul(predictions, positive.t())
            step_loss = F.cross_entropy(logits, torch.arange(N).to(self.device))
            loss2 += step_loss
...
class CLIPInfoNCESpatioFullTrainer(Trainer):
    def __init__(self, encoder, config, device=torch.device('cpu'), wandb=None):
        super().__init__(encoder, wandb, device)
        self.config = config
        self.patience = self.config["patience"]
